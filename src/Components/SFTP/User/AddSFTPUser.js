...
const AddSFTPUser = () => {
    ...
    const [serverAlias, setServerAlias] = React.useState('');
    const [SFTPServers, setSFTPServers] = React.useState([]);
    const [selectedValue, setSelectedValue] = React.useState('password');
    const [selectedValueHostKey, setSelectedValueHostKey] = React.useState('yes');
    const [maximumRetries, setMaximumRetries] = React.useState(6);
    const [connectionTimeout, setConnectionTimeout] = React.useState(0);
    ...
    const createSFTPUserAlias = async () => {
        ...
        try {
            ...
            if (serverAlias !== "" && (SFTPUserAlias.hasOwnProperty("passPhrase") || SFTPUserAlias.hasOwnProperty("password"))) {
                await Axios.post(
                    "http://localhost:5000/api/createSFTPUser",
                    SFTPUserAlias
                ).then((result) => {
                    console.log(result);
